...
source "proxmox-iso" "windows-server-2019" {
  ...
  template_description = "Windows Server 2019 Datacenter Core Packer Template -- ${formatdate("YYYY-MM-DD", timestamp())}"
  ...
  boot_iso {
    ...
    iso_target_path  = "${var.iso_cache_dir}/windows-server-2019-eval.iso"
    unmount          = true
  }
